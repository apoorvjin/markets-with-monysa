...
- Captured: 2026-06-06
- Status: Not started. `sector_treemap.dart`'s tap handler still only opens an `OverlayEntry` tooltip — no `context.push`/navigation to `/asset/:symbol` anywhere in the file.
...
- Captured: 2026-06-06
- Status: Not started. No signal-join logic in `sector_treemap.dart` or `heatmap.ts`; tile color still derives only from `changePercent`.
