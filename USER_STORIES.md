...
> **Audit (2026-07-04):** every story below was checked against the live codebase (not just against prior notes) and got a `Status:` line. Headline findings: US-007b/c already shipped without being marked Done; US-008's target files have grown, not shrunk, since capture; and uncommitted work already in the tree (Firebase auth in `moby/lib/services/auth_service.dart`, `server/routes/admin.ts`, `server/lib/firebase-admin.ts`, `frontend/apps/admin`) has quietly built a real accounts + admin console that changes the scope of US-012/US-013/US-014/US-015 — see their notes for rescoping guidance.
...
- Status: **Done.** Kept the mobile tooltip on tap and added a full-width "View details" button inside `_TooltipCard` (`sector_treemap.dart`) that `context.push('/asset/:symbol?name=...')`. Web already navigated on tile click — unchanged. Interaction is platform-idiomatic (mobile: tap→tooltip→button; web: 1-tap); the *data shown* is identical, satisfying the parity rule.

### US-002 — Strong-buying gold ring on heatmap tiles
**As a** trader **I want** a visual marker (gold ring) on heatmap tiles seeing strong recent buying volume **so that** the heatmap doubles as a "money flowing in now" radar.
- Captured: 2026-06-06
- Completed: 2026-07-22
- Status: **Done, rescoped.** Original ask (join S1/S2/S3 BUY signals) was abandoned: there is **no free, index-aligned BUY-signal source** — no batch signals endpoint, and computing real signals for 500 tiles ≈ 500 Yahoo fetches. Per the user, the ring instead marks **strong buying volume over the last ~30 minutes**: a volume-weighted Accumulation/Distribution money-flow (`computeBuyVolumeSignal` in `heatmap.ts`) over the trailing 30 one-minute bars ≥ 0.30 with window return ≥ 0, computed on the free Yahoo 1m chart. Server sets `buyVolumeSignal` per `TreemapStock`, **1d + REGULAR only, top-50 tiles by market cap** (bounded ≤50 intraday fetches per viewed index per 5m, cached; nothing runs on 1w/1m/ytd or off-hours). Clients render a gold ring (`_StockTile` / `CanvasTreemap`) + a 1D-only legend; mobile tooltip shows a "Strong buying (30m)" chip. Zero-$. Do not re-litigate as "real BUY signals" — none exists for free at index scale.
...
- Captured: 2026-06-06
- Status: Not started. No `treemap_realtime` gate in `entitlement_service.dart`; Finnhub WS still only feeds the quotes poll, not the treemap.
...
- Captured: 2026-06-06
- Status: Not started. No `asOf` param exists in `server/routes/heatmap.ts`.
...
- Captured: 2026-06-07
- Status: Partial — (b) and (c) below shipped and were never marked Done; only (a) remains open. `GET /api/tariffs` serves from `server/data/tariffs.json` (`economy.ts`) and `tariffs_data.dart` calls the API instead of bundling a Dart asset — (b) done. `GET /api/economy/events` is live and the Macro Calendar tab fetches it dynamically instead of hardcoding FOMC/CPI dates — (c) done. `CRISIS_DATA_REVIEWED_AT` in `economy.ts:169` is still a hand-edited string literal (`"June 2026"`), not derived from file mtime/build time — (a) not done.
- Notes: SWOT-identified trust risk. Three offenders: (1) `GET /api/crises` returns `dataAsOf: "May 2026"` — hardcoded in `server/routes/economy.ts`; (2) tariff table in `moby/lib/data/sources/tariffs_data.dart` is an April 2025 snapshot of 113 countries; (3) economic calendar in MacroScreen → Calendar sub-tab (`volatility_screen.dart`) hardcodes FOMC/CPI/NFP/Jackson Hole dates. Fixes: **(a) Crisis playbook — remaining work.** Keep static historical crises (1929, 2008, 2020 don't change) but auto-stamp `dataAsOf` from file mtime or build time, not a string literal; add a "last reviewed" admin note. ~~**(b) Tariffs**~~ — done, see Status. ~~**(c) Economic calendar**~~ — done, see Status. Acceptance (remaining): no user-visible string contains a hand-typed date literal — `CRISIS_DATA_REVIEWED_AT` should be computed, not edited by hand each time CRISIS_DATA changes.
...
- Captured: 2026-06-07
- Status: Not started. Zero matches for form4/13f/etf-flows/unusual/z1/short-interest anywhere in `server/`. Still a valid, large (6–8 week) program — untouched since capture.
...
- Captured: 2026-06-07
- Status: Not started. No `Region`/`regionProvider` in `moby/lib/providers/`; no regional bond/rate endpoints (`/bonds/in`, `/bonds/jp`, `/rates/cn-hk`, etc.) in `server/routes/`. Still a valid, large (8–10 week) program — untouched since capture.
...
- Captured: 2026-06-07
- Status: Not started. No `/api/crypto/*` routes or `cryptoTickerHub` anywhere in `server/`. Still a valid, large (6–8 week) program — untouched since capture.
...
- Captured: 2026-06-07
- Status: Not started as spec'd — no `docs/licensing/`, `LICENSING.md`, or `UsageMeter` exist. **Rescope before starting:** uncommitted work has already built a real admin console (`server/routes/admin.ts`, `server/lib/firebase-admin.ts`, `frontend/apps/admin`) with its own `authMiddleware`, including `/api/admin/cache/bust` and `/api/admin/ai-usage`. Build this story's `UsageMeter`, vendor-health probe, and kill-switch endpoints as new routes inside that existing admin app/auth scheme instead of introducing a separate `ADMIN_TOKEN` Bearer scheme — otherwise you end up with two parallel admin auth systems.
...
- Captured: 2026-06-07
- Status: Not started as spec'd — no `server/llm/` directory, `LlmQuota`, `LlmBudget`, or `callLlm()` helper exist. **Rescope before starting:** `/api/admin/ai-usage` already exists in the new `server/routes/admin.ts`, giving basic spend visibility ahead of schedule. Extend that endpoint and the existing admin auth (`authMiddleware` in `admin.ts`) rather than inventing a parallel `ADMIN_TOKEN` scheme per this story's original spec (see also US-012's rescope note — both stories should share one admin auth path).
...
- Captured: 2026-06-07
- Status: Partial, and needs rescoping. The desktop-UX pillar shipped as `frontend/apps/web` — but as a **Vite SPA** (React 19 + TanStack Router/Query), not the Next.js SSR/SSG app this story prescribes. That means the entire programmatic-SEO pillar (the story's primary strategic bet — `/politicians/[slug]`, ISR, sitemaps, JSON-LD) does not exist and cannot be retrofitted onto a client-rendered SPA without a separate SSR framework or a second app. Separately, the "hard prerequisite — accounts" section is now moot as originally scoped: Firebase email/password auth (`moby/lib/services/auth_service.dart`) plus `server/lib/firebase-admin.ts` are already shipping as the real accounts system, not the magic-link/JWT scheme this story describes. **Recommendation:** treat the SEO/acquisition pillar (sections 1 "monysa.com" and 4 "Programmatic SEO content quality") as a distinct, smaller follow-up story if organic acquisition is still a priority — it would need its own SSR-capable app (Next.js, Astro, or a static-gen pass) since it can't share the existing Vite SPA. Drop or rewrite the "Auth & accounts" section to describe Firebase auth instead of magic-link JWT. The `app.monysa.com` desktop-UX pillar can be marked mostly satisfied by the current `frontend/apps/web` build.
...
- Captured: 2026-06-07
- Status: Not started, and the case has gotten stronger. As of 2026-07-04 the files have grown, not shrunk: `trading_screen.dart` is 3815 LoC (was 3260), `volatility_screen.dart` 3422 (was 3070), `investing_screen.dart` 3176 (was 2330), `tenx_backtest_screen.dart` 1489 (was 1260). No `features/*/tabs/` directories exist anywhere yet.
...
### US-019 — Live signal track record + paper portfolio
<!-- Was mis-numbered US-007 (duplicate ID with the stale-content-refresh story above); renumbered 2026-07-04. -->
...
- Captured: 2026-06-07
- Status: **Phase 1 shipped** (silent accumulation), Phases 2–3 not started. Implementation diverged from the notes below: `server/lib/signal-ledger.ts` + `lib/exit-detection.ts` writing Firestore collections `signal_ledger` / `signal_ledger_stats` — not a `signal_log` store in `server/data/`; kill switch is the Remote Config flag `signal_ledger_capture_paused`, not an `ENABLE_SIGNAL_LOGGER` env var; cadence is nightly (capture 00:25 UTC, resolve 00:40 UTC, leader-gated), not 15-min. Scope is `1d` only, 49 assets × 16 strategies (+S9/S18 on SI=F). No `/api/trading/track-record*` endpoints exist yet and no client reads either collection — Phase 2 still to build.
- Ledger data caveat (2026-08-19 audit): **v1 data is not usable and must not be published.** Two distortions. (1) v1 anchored each trade to the live quote while resolving against daily candles, so ~15% of entries — concentrated in 24h futures (CT=F 90%, SI=F 76%, HG=F 67%, GC=F 57%) — were priced off a bar they were never measured against and resolved at a 0% win rate. Fixed in v2 by anchoring to the entry bar's close and shifting SL/TP by the same delta (`STRATEGY_LOGIC_VERSION` bumped 1→2, so v1 is frozen in its own seriesKey and never blended in). (2) Win rate stays censored until a cohort completes `maxHoldBars` (20 bars): stops sit ~2.3% from entry vs ~5.7% for targets, so the near barrier resolves first and the resolved sample is SL-enriched. **Read `exitTIMEOUT` on the stats doc first — while it is 0, no cohort has completed and the win rate is meaningless.** Max drawdown is deliberately not in the rollup (path-dependent); compute it at read time over `signal_ledger` ordered by `resolvedAt`.
...
- Captured: 2026-06-08
- Status: Not started. Reconfirmed 2026-07-04: no `currency_format.dart` exists; `trading_screen.dart`'s `_formatPrice` still ignores `QuoteItem.currency`, and its `_currencySymbol` still returns `''` for `'USD'`. All bugs listed below are still live.
...
- Captured: 2026-06-08
- Status: Not started. Redis/Upstash usage as of 2026-07-04 is still confined to `oge.ts` and `leader.ts` — none of the priority routes (signals, futures, treemap, scanner, quiver) have the L2 pattern.
...
- Captured: 2026-06-08
- Status: Not started. `_noteCache` (now `server/trading.ts:3855`) is still a plain in-memory `Map` with no Redis reference nearby.
...
- Captured: 2026-06-08
- Status: Not started as spec'd, but now cheap to finish. `/api/admin/cache/bust` already exists in the new `server/routes/admin.ts` with its own `authMiddleware` — add `POST /api/admin/tariffs/refresh` as a sibling route reusing that same auth instead of introducing a separate `ADMIN_TOKEN` scheme (same rescoping note as US-012/US-013).
...
### US-020 — Register Sign in with Apple email relay so Hide-My-Email delivers
**As a** user who chose "Hide My Email" during Apple sign-in **I want** the verification and notification emails Finbrio sends to actually reach my inbox **so that** I can verify my account and receive alerts without exposing my real address.
- Captured: 2026-07-17
- Status: Not started. Blocks nothing today (Apple sign-in works), but any user who picks Hide My Email gets a `…@privaterelay.appleid.com` address that will silently bounce until the relay is configured.
- Notes: Apple relays mail sent to `@privaterelay.appleid.com` addresses **only** from sender domains/addresses registered under **Apple Developer → Certificates, Identifiers & Profiles → More → Configure Sign in with Apple for Email Communication**. Steps: (1) add the sender address/domain we send from (`noreply@finbrio.net` — matches `RESEND_FROM_ADDRESS`; see [server/routes/auth.ts](server/routes/auth.ts)) to that Apple config; (2) ensure `finbrio.net` SPF/DKIM are already verified in Resend (they are, per the verification-email work); (3) send a test verification email to a Hide-My-Email account and confirm delivery through the relay. Until done, Hide-My-Email users can sign in but won't receive the FinBrio verification email (`POST /api/auth/send-verification-email`). Alternative/adjacent: add an in-app "confirm your email" step when `user.email` is null or a private relay (see auth flow in [moby/lib/services/auth_service.dart](moby/lib/services/auth_service.dart)). Acceptance: (1) sender address registered + shows "Verified" in Apple Developer; (2) verification email sent to a relay address lands in the real inbox; (3) reply-path stays within the registered domain (Apple drops relayed mail from unregistered senders).

### US-021 — Cross-Signal Cluster engine (MVP2): Pro "Signals" tab over live smart-money sources
**As a** Pro subscriber **I want** a Signals tab that surfaces tickers where multiple independent "smart money" indicators converge (insider buying + rising lobbying spend + presidential trades) **so that** I can spot high-conviction setups without manually cross-referencing the Smart $, Presidential, and insider data myself.
- Captured: 2026-07-17
- Target: MVP2
- Status: Not started. Planned. See plan file `finbrio-zero-cost-backend-sunny-crown.md`.
- Notes: Consolidates an external "FinBrio Zero-Cost Backend" brief after finding it unbuildable as written — it assumed a Firebase Cloud Functions + Firestore-trigger backend (reality: Express on Fly.dev; Firestore is `firebase-admin`-only), a Flutter `cloud_firestore` StreamBuilder (reality: Dio→Express + Riverpod + plan enforcement), congress-trade data (dead since 2026-07 — Quiver 401s, FMP empty, House Stock Watcher feed dead), a `insider_trades` store to migrate (reality: `/api/quiver/insider` is an on-the-fly top-10 aggregation), and a 10-K supplier-extraction pipeline with hardcoded guessed tickers (violates the anti-fabrication rule; no pipeline exists). **Scope (agreed):** single `cross_signal_cluster` signal type joining the three confirmed-live sources per ticker — SEC EDGAR Form 4 insider clusters (`fetchInsiderClusters()`), Senate LDA lobbying growth (`getLobbyingBadgeMap()`), Presidential OGE trades (`/api/oge/trump-transactions`) — surfacing tickers with `score >= 2`. Congress-ready but excluded until a live source exists. **Delivery:** new Pro-gated `GET /api/signals` in `server/routes/signals.ts` (reuse quiver.ts helpers + `resolveTickerForName()`; in-process/Redis cache; `private` Cache-Control); web parity via `packages/contracts` zod schema + api-client; Flutter `_SignalsTab` in [moby/lib/features/investing/investing_screen.dart](moby/lib/features/investing/investing_screen.dart) modeled on `_QuiverTab`, gated via `EntitlementService.can()`, tapping a card → `/asset/:symbol`. **No fabricated entry/SL/TP.** Cost ~$0 (reuses Fly + existing `ANTHROPIC_API_KEY` for optional interpretation polish). Purely additive — no existing endpoint/route changes; live TestFlight + web unaffected. Acceptance: (1) `curl /api/signals` returns clusters all with `score>=2` and resolved company names, no trade-setup fields; (2) free device gets `403 PLAN_REQUIRED`; (3) web + Flutter Signals tabs render identical data (parity rule); (4) free-plan Flutter shows `UpgradeSheet`.

### US-022 — SPLC: resolve anonymized counterparties from filing text (Haiku)
**As a** user browsing a company's supply chain **I want** rows that currently read "Undisclosed customer" to name the actual company where the filing states it elsewhere **so that** I can see who the relationship is with instead of only that one exists.
- Captured: 2026-08-15
- Status: Not started. Deliberately deferred from the SPLC v1 build.
- Notes: Reg S-K requires disclosing a >10% customer but **not** naming it, so the XBRL member is usually a placeholder (`CustomerA`, `TenLargestCustomers`). Measured on one month of SEC Notes data: **954 of 1,256 usable facts (~76%) are anonymized**. These are currently ingested and shown as an italic, non-clickable "Undisclosed customer" row carrying the real percentage (confidence 0.7) — the concentration is genuine, only the identity is withheld. The name is frequently stated in prose in the *same* 10-K ("Walmart accounted for approximately 22% of net sales"), so a Haiku pass over Item 1 / Item 7 / the Concentrations note can often recover it. **This is the highest-fabrication-risk piece of SPLC and must not ship without a review queue** — an LLM mapping `CustomerA` to a company name is a guess presented as data, which is exactly the failure mode [US-021](#us-021) and the anti-fabrication rule exist to prevent. Required design: extracted names land in an `unresolved_entities` review collection at a *lower* confidence tier (≤0.85 per the SPLC confidence scale), never auto-promoted to a `Disclosed` row, and never used to create a graph centre until a human confirms. Cost: ~$3–8/mo on Haiku 4.5 at ~4,000 filings/yr (`ANTHROPIC_API_KEY` already configured; same pattern as `routes/tariff-refresh.ts`). Acceptance: (1) a resolved counterparty renders with its own confidence chip, visually distinct from a filer-stated `Disclosed` row; (2) unconfirmed extractions are never shown as a named company; (3) an admin surface lists pending extractions with the source filing excerpt; (4) rejecting an extraction reverts the row to "Undisclosed customer"; (5) resolution never creates a new SPLC universe entry on its own.

### US-023 — SPLC: EDGAR full-text mining for filers who never tagged XBRL
**As a** user **I want** supply-chain relationships from companies that describe a customer concentration in prose but never tagged it in XBRL **so that** coverage isn't limited to filers with disciplined tagging practices.
- Captured: 2026-08-15
- Status: Not started. Deferred from the SPLC v1 build.
- Notes: SPLC v1 reads only structured XBRL facts from the SEC Notes Data Sets, so a company that writes "our largest customer accounted for approximately 31% of revenues" without tagging `ConcentrationRiskPercentage1` is invisible to it. EDGAR full-text search (`https://efts.sec.gov/LATEST/search-index`) is free, keyless, and indexes back to 2001 — phrase targets: `"accounted for approximately"`, `"of our net revenue"`, `"our largest customer"`, `"sole source"`, `"single supplier"`. Operational constraints are real and already documented in the SPLC brief: a `User-Agent` with org + email is mandatory (403 otherwise), ≤10 req/s across all EDGAR hosts, `/LATEST/` must be uppercase, CIKs must be 10-digit zero-padded (`ciks=320193` → HTTP 500), and there is a 10,000-result deep-pagination ceiling so queries must be windowed by date and stitched. Exhibits are indexed separately, so dedupe on accession number. This is the widest-reach expansion but shares US-022's risk profile — every extracted figure is LLM-parsed prose, so it must inherit the same review queue and confidence tier rather than landing in the graph directly. Sequence **after** US-022 so the review surface exists first. Acceptance: (1) phrase-mined edges carry a distinct method and confidence below any XBRL-sourced edge; (2) rate limiting and the UA header are enforced centrally; (3) no phrase-mined edge creates a graph centre without confirmation; (4) date-windowed pagination is verified against a query exceeding 10,000 hits.
