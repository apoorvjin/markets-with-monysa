...
**Target users**: The macro-curious prosumer retail investor (follows tariffs, congressional trades, and the debt clock and wants it consolidated) plus self-directed active traders. (Softened 2026-08 from the older "professional traders / trade-compliance teams" framing — copy only; marketing personas structurally unchanged. See marketing site `index.astro`.)
...
server/
  ...
  routes/               # Modular route files registered by index.ts
    ...
    splc.ts             # GET /api/splc/universe, GET /api/splc/:symbol — Supply Chain Analysis.
                       #   Serves ONLY pre-computed Firestore docs (see lib/splc/) — never computes
                       #   at request time. `no-cache` on both: the nightly batch can change the
                       #   payload's contents AND its shape, and a client holding a long-max-age copy
                       #   has no way to discover that (this shipped broken once — see Known Pitfalls).
  ...
  lib/                  # Shared server utilities
    ...
    splc/               # Supply Chain Analysis (SPLC) — who a company sells to / buys from.
      sec-fetch.ts      #   THE single throttled entry point for every SEC request (~8 req/s, <10/s
                       #   ceiling, 4 concurrent, 429/503 backoff). SEC enforces by blocking the IP,
                       #   which would also kill quiver.ts's EDGAR calls — never call fetch() to
                       #   sec.gov directly from SPLC code.
      sec-notes-ingest.ts  # Financial Statement *and Notes* Data Sets (NOT the plain ones — see
                       #   Known Pitfalls). Extracts customer/supplier concentration facts.
      entity-resolution.ts # ticker <-> CIK <-> company name (exact, then unique-prefix; never fuzzy-guesses)
      derivation.ts     #   two-sided quantification + Σ<=1.0 constraint checks
...
AISSTREAM_API_KEY                  optional — aisstream.io free WebSocket key for the Tankers web pillar's live AIS
                                              vessel feed (leader-gated persistent WS → in-memory vessel map →
                                              GET /api/maritime/vessels + /api/maritime/chokepoints; the stream IS the
                                              cache, like /api/trading/quotes). Absent = feed never starts and those
                                              routes return source:"cold". aisstream is beta/no-SLA with recurring
                                              outages — the provider seam in server/lib/ais-feed.ts (selectProvider)
                                              lets a paid satellite provider (Datalastic/VesselFinder) swap in for
                                              mid-ocean coverage without touching anything downstream.
AIS_PROVIDER                       optional — which AIS provider server/lib/ais-feed.ts uses (default "aisstream").
```
...
moby/lib/
  ...
  app.dart                         # MaterialApp.router (title: 'FinBrio') + AppShell (bottom nav, 6 tabs)
  ...
  features/
    ...
    wire/wire_screen.dart          # WireScreen — News/OSINT terminal (5th nav tab, before Profile). Desk chips
                                   # (10 desks) + cross-desk breaking banner + headline list; tap headline → source
                                   # article (url_launcher), tap Corporate-desk ticker chip → /asset/:symbol. Read-only
                                   # (no push yet). Mobile port of web features/wire/WirePage.tsx — keep desk order +
                                   # severity/category mapping in sync (data-display parity). Models: data/models/wire.dart;
                                   # data: data/repositories/wire_repository.dart (8m items / 60s breaking client TTLs).
...
/wire                      → WireScreen            (features/wire/wire_screen.dart; News/OSINT terminal)
...
- **Routes** (mirror mobile tab structure): `/markets` (Heatmap/Indices/Commodities/Forex/CFTC), `/trading` (Instruments/Dashboard/Power Moves/Signals S1–S9/Alerts — watchlist+alerts in localStorage), `/investing` (Exposure/Dashboard/Multibaggers/Presidential/Smart $/Earnings Calendar/ETFs — Congress/House Trades removed 2026-07; a "Supply Chain" sub-tab existed briefly and was replaced by the dedicated `/splc` page), `/splc` (SPLC Analysis — see below), `/macro` (Dashboard w/ regime+gauges+heatmaps+yield graph+RRG quadrants+AI briefing / Crisis / Debt / Calendar / Correlation), `/asset/$symbol?name=`.
- **Known deliberate divergence (2026-08): SPLC Analysis (`/splc`) is web-only.** It was scoped as web-first; no Flutter port exists. The server (`lib/splc/`, `routes/splc.ts`) and the `@monysa/contracts` schemas are platform-neutral, so a mobile port is additive — it needs a screen + repository, no backend work. Until then this is the one place web is ahead of mobile; don't read the parity rule below as "SPLC is missing/broken on mobile".
- **Trading rebuilt as a four-step funnel (2026-08)**: `/trading` is now `?tab=scan|evaluate|track|act` (plus `&sym=` for the symbol
  carried between steps), landing on **Evaluate**. The old five tabs were rehoused, losing nothing: Power Moves + Best Setups + Win-Rate
  Leaderboard + Top Movers → **① Scan**; Signals → **② Evaluate**; Instruments + Watchlist snapshot → **③ Track**; Alerts → **④ Act**.
  `DashboardTab`, `PowerMovesTeaserCard` (it teased a tab that is now step ①), `SignalsTab` and `SignalRow` were deleted.
  **② Evaluate is a triage queue**: BUY/SELL first, HOLD collapsed behind a count, ranked by confidence.
  **Reward-to-risk is deliberately NOT the ranking key.** The server derives stopLoss/takeProfit as fixed bands around entry, so
  `riskReward()` (lib/riskReward.ts) returns a constant **2.5 on every actionable signal** (measured spread across 12 signals: 0.024).
  Sorting by it would sort on rounding noise and printing it per row would imply a difference between setups that does not exist, so it is
  stated once — "Every setup targets 2.5 : 1" — and the per-row column plus the sort option are gated behind a **runtime variance check**
  (`rrVaries`), not a hard-coded assumption: vary the bands server-side and both reappear on their own. Don't "fix" this by re-adding an
  R:R sort.
  **② Evaluate has two modes.** With no `?sym=` it is the triage queue (one request per asset). With a symbol carried from Scan it
  switches to `FocusedEvaluation`: that ONE symbol rated by every base strategy the viewer can see — **3 requests on free, 9 on Pro,
  instead of 49** — plus a consensus line ("3 of 3 strategies say HOLD"). Scoring the other 48 assets to answer a question about one
  symbol was both wasteful and the wrong question; don't "restore" the full fan-out for the focused case. Enhanced (`+`) strategies are
  excluded (refinements of the same nine, 18 rows reads as noise) and gated ones are skipped rather than fetched-then-blurred.
  **Rate limit**: the queue mode fans out one `/api/trading/signals/:symbol` request per asset — 49 against the signals limiter's 60/min
  (`server/index.ts` `signalsLimiter`) — and it is now the landing tab, so switching strategy inside the same minute can 429. Mitigated
  client-side with bounded retry + backoff, and failures are surfaced ("Couldn't score N of 49") rather than silently shrinking the
  actionable count. **The real fix is a batch signals endpoint** — not built, would be a backend change.
- **Markets sub-tab revamp (2026-08)**: Indices/Commodities/Forex share `MarketTable`, which now carries an advancing/declining
  breadth strip, category quick-filter chips derived from the feed's own `category` values (selecting one suppresses group headers,
  since there is then only one group), sticky group dividers, and **keyboard-navigable rows** — a bare `onClick` on `<tr>` is mouse-only,
  so rows are `role="button" tabIndex={0}` with Enter/Space handling and a focus ring. Commodities' second column is labelled **Unit**
  (it holds "per troy oz", never a region). Heatmap's 17 index chips are grouped by `region` (a field added to `TREEMAP_INDICES` in
  `@monysa/contracts`, web-only — mobile keeps its flat list) and the tab shows constituent breadth plus a hint that sector headings
  drill in. CFTC gained a "What is this?" explainer, a dedicated **Positioning** column (split bar + "N% long") replacing a `%`
  stacked awkwardly on a bar inside a numeric cell, and lost the heading that merely repeated the active chip.
  **Pro teasers**: `.cftc-row-blur` and `.fx-diff-blur` overlays were changed from the shared 55% colour wash to a light scrim plus a
  small lock chip — across a 40-row forex table that wash turned every gated row into what looked like a solid CTA button. Scoped to
  those two Markets-only classes; the inline ETF/correlation/briefing teasers elsewhere keep the original treatment.
- **Markets UX revamp (2026-08)**: `/markets` sub-tabs live in the URL (`?tab=overview|heatmap|indices|commodities|forex|cftc`, plus
  `&index=`/`&tf=` for the Heatmap and `&cot=` for CFTC) via `validateSearch` on `marketsRoute` — so a tab is linkable, survives refresh,
  and answers the back button. `validateSearch` returns an **optional** key (`{ tab?: MarketTab }`, `{}` when absent), not
  `{ tab: undefined }` — the latter forces every existing `navigate({to:"/markets"})` in the app to pass `tab` explicitly. `MarketsPage`
  re-validates `?tab=` itself as well, because an unrecognised value otherwise renders Overview's content while lighting up no tab at all.
  Tabs render as an underlined `.mk-tabs` bar on their own row (`role="tablist"`, `aria-selected`, focus ring); the old right-aligned pill
  chips read as filters, not navigation. The "Dashboard" tab became **Overview** (`features/markets/OverviewTab.tsx`): a session strip, an
  index rail with sparklines, the global-indices table, ranked diverging sector bars (1D/1W/1M), real S&P 500 movers, a breadth bar from
  `/api/trading/regime-summary`, and the full 3M/5Y/10Y/30Y curve. It replaced `components/DashboardGrid.tsx` + `components/MetricCard.tsx`
  (both deleted) whose donut hardcoded `value: 1` for all 11 sectors — encoding nothing — and whose "Top Gainers" ranked the 46 world
  indices, so the top gainer was routinely the VIX. Exchange-session logic now lives once in `lib/sessions.ts`, shared by the nav's
  `MarketStatus` pill and the Overview strip. **Overview is web-only and deliberately so** (its predecessor was too) — the other five
  sub-tabs stay in data-display parity with the Flutter Markets tab.
- **Markets tab parity (2026-08)**: the web Markets page was brought up to the Flutter tab after it had drifted. `MarketTable` now carries
  the sortable header (Index/Value/% 1D · Commodity/Price/% Chg · Asset/Price/% Chg — click to sort, click again to flip; "relevance" is a
  deliberate no-op that preserves the server's curated market-cap/liquidity order), category grouping for Indices and Forex, an inline 1M
  sparkline for Indices and Commodities, and the "No results" empty state. Defaults mirror the Flutter tabs exactly: Commodities opens sorted
  by % change descending, Indices and Forex open on the server order. Forex drops the Region and Trend columns (FX rows have neither field) and
  reveals the rate-comparison label on exactly **one** row across the whole grouped list, not one per group. The Heatmap tab gained the
  `marketState` session pill (Live / Pre-market / After-hours / Overnight / Closed — only `PRE` and `POST` are live sessions), the
  "Showing N of M resolved" short-map line, and a "What is this?" explainer whose colour legend is generated from `tileColor()`. CFTC gained
  the Long/Short contract columns and the long/short split bar, and prints the CFTC report week once under the list instead of repeating it
  on every row. `MarketItem` in `@monysa/contracts` now declares `category`/`unit`/`base`/`quote`/`sparkline` (the server always sent them;
  they were reaching the client through `.passthrough()` untyped).
...
| Nav tab count | 5 tabs | 6 tabs (Market/Trading/Investing/Macro/**Wire**/Profile) — Wire added 2026-08 as the 5th slot, before Profile. The old "5-tab invariant" is retired; `_tabs` in app.dart is the source of truth. |
...
| Treemap index count | 5 (S&P 500/NASDAQ 100/DJI/FTSE 100/Nifty 50), or 9 | **17** — the 9 above (S&P 500, NASDAQ 100, Dow Jones, Russell 2000, FTSE 100, DAX 40, Nikkei 225, Hang Seng, Nifty 50) plus the Wave-2 additions TSX 60, ASX 50, Ibovespa, IPC Mexico, KOSPI, TAIEX, JSE Top 40, STI. Source of truth: `_kIndices` (treemap_tab.dart) + `TREEMAP_INDICES` (@monysa/contracts heatmap.ts) — both carry a flag emoji on every label; keep the two lists in sync with `server/data/index_constituents.ts`. |
...
| CFTC category chip order | Metals/Energy/Agriculture/Currencies/Indices & Rates | Metals/Energy/**Indices & Rates**/Agriculture/Currencies/**Regional Flows** (2026-08) — reordered/extended; keep mobile (`_chips` in markets_screen.dart) and web (`COT_GROUPS`/`REGIONAL_FLOWS_KEY` in MarketsPage.tsx) in sync |
| CFTC COT is US-only — don't add more "global CFTC" index rows | adding DAX/Euro Stoxx 50/FTSE 100/Hang Seng/ASX 200 COT rows because the theme is "global" | CFTC only regulates US exchanges; its COT dataset has **zero** rows for Eurex, ICE Futures Europe, HKEX, NSE, or ASX contracts (confirmed by querying `publicreporting.cftc.gov` directly — no amount of searching finds them, it's a regulatory-scope limit, not a data gap). The **only** non-US index CFTC reports on is **Nikkei 225**, because CME dual-lists it (Yen-denominated, code `240743`, symbol `NIY=F`) — that's why it's the one addition in `COT_LEGACY` (server/routes/markets.ts). The sibling USD-denominated contract (code `240741`) stopped getting fresh weekly reports (stale since 2026-03) and is deliberately excluded. If a future ask wants more non-US markets on this page, they need a genuinely different free source per exchange (see next row), not a CFTC lookup — there isn't one. |
| NSE FII/DII "Regional Flows" section | assuming it's COT-equivalent long/short futures positioning | It's a **different metric**: NSE's daily *cash-market* FII vs DII net buy/sell in ₹ crores (`fetchNseFiiDiiFlows()` in markets.ts, hits `nseindia.com/api/fiidiiTradeReact`), not futures contracts long/short. Kept in its own `regionalFlows` response field and its own UI section (cards, not the COT table) on both platforms — never merge it into a `CotItem`/`CotMetal` row. The NSE endpoint is unofficial (no auth token needed in testing, but anti-bot protection could tighten any time) — `getRegionalFlows()` fails soft to the last good cached value rather than blanking the section or throwing. |
...
| Tall filter/chip header eating permanent screen space above a scrollable list | pinning the filter row as a fixed sibling above the list (`Column: [header, Expanded(list)]`) | **"Scroll-Away Filters" pattern**: put the filter/chip header as the *first item inside the same scroll view* as the results (`CustomScrollView`'s first `SliverToBoxAdapter`, or the first child of the scrollable `Column`) instead of a pinned sibling — it then scrolls out of view with the content on scroll-down and back into view on scroll-up, for free, with no `ScrollController`/`AnimatedContainer`/`SliverPersistentHeader` needed. No shared widget exists (it's a layout choice, not a component) — reapply the same restructuring per screen. Live examples: Trading → Signals (`_SignalsTabState.build`, trading_screen.dart, `CustomScrollView`) and Markets → Heatmap (`_TreemapTabState.build`, treemap_tab.dart, `SingleChildScrollView`). Caveat: any height math derived from the *remaining* viewport (`LayoutBuilder` sized to fill space below the old fixed header) must be recomputed against the *full* viewport once the header moves into scroll flow, and non-scrolling content built with `Expanded`/flex (e.g. a loading skeleton) needs a `SizedBox`-bounded height instead, since `Expanded` requires bounded constraints and a scroll axis is unbounded. |
...
| Summing concentration shares across periods | dedupe to the latest period *per counterparty* | Choose one cohort per (filer, side): latest `ddate`, longest `qtrs`, then keep only rows on that basis. Picking per-counterparty lets a 12-month share sum against a 9-month one — that mismatch, not real over-disclosure, caused 33 of 51 flagged groups. Also exclude anonymised members from the Σ≤1.0 check: `TenLargestCustomers` / `CustomerAandB` are deliberately supersets of the named rows beside them and guarantee false violations. |
| `session` vs `marketState` casing on `/api/heatmap/movers` | `if (session === "POST")` | The same payload carries **`session: "post"` (lower)** and **`marketState: "POST"` (upper)**. The server RANKS `gainers`/`losers` by the active session's move, so a case-sensitive miss silently falls back to `changePercent` and the card renders an after-hours gainer showing its regular-close *loss* — green up-arrows on negative numbers. Always `.toUpperCase()` before matching, display the same field the server ranked by, and derive the row's arrow/colour from the number actually on screen rather than from which array it arrived in. |
| Passing `var(--token)` as a canvas colour | `<Sparkline color="var(--positive)" />` | Canvas `strokeStyle`/`fillStyle` do **not** resolve CSS custom properties — the assignment is silently ignored and the line draws in the default black, i.e. invisible on the dark theme, with no error anywhere. `Sparkline` already resolves `--positive`/`--danger` itself via `getComputedStyle`; pass `positive={bool}` to pick between them (colour by today's % change) or omit it to colour by the window's own trend. Same rule for any other canvas chart in `@monysa/charts`. |
| Hard-coding swatch hexes for a chart legend | copying mobile's `_LegendRow` colours into the web legend | Mobile's treemap (`sector_treemap.dart`) and web's (`CanvasTreemap`) use **different** colour scales — web interpolates NEUTRAL→`#00d4aa`/`#ff4d6a` clamped at ±3%, so mobile's `#8A1A1A`/`#0B6B35` palette is simply wrong there. Build the legend from the chart's own exported `tileColor(change)` so it cannot drift from what the tiles are painted with. |
| `ProBlur` on a block-level element | `<ProBlur className="my-thing">` with no CSS rule for it | `.pro-blur` is `display: inline-flex` (built for inline chips). Every block-level use needs its own override — see `.cftc-row-blur`, `.treemap-blur`, `.splc-col-blur` — or it collapses to a narrow box. Typecheck cannot catch a missing CSS class; only looking at the rendered page does. Also cap `max-height` on tall blocks or the centred "Upgrade to Pro" label drifts off-screen. |
| Changing a persisted API response shape | bumping only the server, or telling users to hard-refresh | Three caches must all be considered: the browser **HTTP cache** (a previously-served `max-age` stays "fresh" and the browser won't even ask — a hard refresh bypasses it, `localStorage` is untouched), **`persistQueryClient` in localStorage** (needs the `buster` bumped — a hard refresh does NOT clear it), and **in-memory React Query state** (`staleTime`/`refetchOnMount`). Versioning the query key is the reliable escape hatch for an already-poisoned client. This shipped broken twice before all three were addressed. |
...
| **Wire** `/wire` | News/OSINT terminal (5th nav tab). Desk chips (10 desks: Intel/World/Middle East/Europe/Africa/Latin America/Asia-Pacific/United States/Markets & Macro/Corporate Wire) + cross-desk breaking banner + newest-first headline list. Tap headline → source article (external browser); tap Corporate-desk `$TICKER` chip → `/asset/:symbol`. Pull-to-refresh. Read-only (no push yet). Mobile port of web `WirePage.tsx`. | `/api/wire/items` `/api/wire/breaking` (`/api/wire/desks` available; mobile uses the static desk enum) | **All free.** |
| **SPLC Analysis** `/splc` (web only) | Supply Chain Analysis — Bloomberg-SPLC-style board. Search a company by **name or symbol**, then: suppliers left (who sells TO it), the company centre, customers right (who buys FROM it). Tapping a counterparty that has its own page re-centres on it. Values render as `$412M (22.0%)`. Every row is labelled `Disclosed` / `Derived` / `Federal contract`, or shown as an italic non-clickable *Undisclosed customer* when the filer withheld the name. A "What is this?" explainer sits in the header. **No mobile equivalent — see the parity note below.** | `/api/splc/universe` `/api/splc/:symbol` | **Free**: first 5 rows per column. **Pro**: the full graph. Real `useIsPro()` gate, not a permanent teaser. |
