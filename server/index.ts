...
function setupRateLimiting(app: express.Application) {
  ...
  // Caps outbound email volume per device — each call sends a real email via Resend.
...
<h2 style="font-size:16px;font-weight:600">Is FinBrio available on Android?</h2>
<p>iOS only at this time.</p>
<h2 style="font-size:16px;font-weight:600">How do I cancel my subscription?</h2>
<p>Open the Settings app on your iPhone → Apple ID → Subscriptions → FinBrio → Cancel.</p>
<hr>
<p style="font-size:13px;color:#888">© 2026 FinBrio. All content is for informational purposes only and does not constitute investment advice.</p>
</body>
</html>`);
  });
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>FinBrio — Privacy Policy</title>
<style>
  body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:680px;margin:48px auto;padding:0 24px;color:#1a1a1a;line-height:1.6}
  h1{font-size:28px;font-weight:700;margin-bottom:4px}
  h2{font-size:18px;font-weight:600;margin-top:36px}
...
<hr>
<p>FinBrio ("we", "our", or "us") is committed to protecting your privacy. This policy explains what data we collect, how we use it, and your rights.</p>
...
<p>FinBrio is not directed at children under 13. We do not knowingly collect data from children.</p>
...
<h2>9. Contact</h2>
<p>FinBrio<br><a href="mailto:apurva.ggps@gmail.com">apurva.ggps@gmail.com</a></p>
<hr>
<p style="font-size:13px;color:#888">© 2026 FinBrio.</p>
</body>
</html>`);
  });
  ...
  // manual-post-only). Safe with no config — runs in DRY_RUN, queues
  // candidates for admin review, never calls a real API.
  const { startSocialBuzzPoller } = await import("./lib/social-buzz/poller");
  startSocialBuzzPoller();
